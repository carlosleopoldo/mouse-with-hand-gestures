...
def find_finger_tip(processed):
    if processed.multi_hand_landmarks:
        hand_landmarks = processed.multi_hand_landmarks[
            0
        ]  # Asumimos que solo una mano es detectada
...
def is_left_click(landmark_list, thumb_index_dist):
    return (
        util.get_angle(landmark_list[5], landmark_list[6], landmark_list[8])
        < 50  # Ángulo entre puntos clave de los dedos
        and util.get_angle(landmark_list[9], landmark_list[10], landmark_list[12]) > 90
        and thumb_index_dist > 50  # Distancia entre el pulgar y el índice
    )
...
def detect_gesture(frame, landmark_list, processed):
    if (
        len(landmark_list) >= 21
    ):  # Verificamos que se hayan detectado suficientes puntos
        ...
        if (
            ...
            and util.get_angle(landmark_list[5], landmark_list[6], landmark_list[8])
            > 90
        ):
...
def main():
    ...
    )  # Utilidad para dibujar las conexiones de las manos
            ...
            if processed.multi_hand_landmarks:
                ...
                for lm in hand_landmarks.landmark:
                    landmark_list.append(
                        (lm.x, lm.y)
                    )  # Almacenamos las coordenadas normalizadas
